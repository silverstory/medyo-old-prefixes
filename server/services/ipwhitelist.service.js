...
const postIpwhitelist = async (req, res, next) => {
  ...
  if ( find_ipwhitelist != null ) {
    ...
    const updated_ipwhitelist = await Ipwhitelist.findByIdAndUpdate({_id: find_ipwhitelist._id}, _ipwhitelist, { new: true });
...
const deleteIpwhitelistByIp = async (req, res, next) => {
  try {
    ...
    const _ipwhitelist = req.body;
    let ipwhitelist = null;
    const cursor = await Ipwhitelist.find({ip: _ipwhitelist.ip}, { _id: 0 }).limit(1).cursor();
    ipwhitelist = await cursor.next();
    if ( ipwhitelist != null ) {
